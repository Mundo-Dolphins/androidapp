...
class ArticlesViewModelMockWebServerTest {
    ...
    @Test
    fun `fetchArticlesSuspend should update state when server returns data`() =
        runTest {
    ...
    @Test
    fun `fetchArticlesSuspend should return false and not update state on server error`() =
        runTest {
            val server = MockWebServer()
            server.start()
