...
// Function to configure contract validation once the base test task is available
project.afterEvaluate {
    val testDebugTask = tasks.findByName("testDebugUnitTest") as? Test
    if (testDebugTask != null) {
        tasks.register<Test>("validateApiContracts") {
            group = "verification"
            description = "Validate API contracts (schemas and examples)."
            testClassesDirs = testDebugTask.testClassesDirs
            classpath = testDebugTask.classpath
            filter {
                includeTestsMatching("es.mundodolphins.app.contracts.*")
            }
        }
    }
}
